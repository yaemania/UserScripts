...
$iv(function() {
  $iv("#sidebar div:contains('Comparte con Nosotros')").remove();
  //$iv("#sidebar div:contains('Identificación')").remove();
  $iv("#sidebar div:contains('Recibenos en tu Correo')").remove();
  $iv("#sidebar div:contains('Webs Amigas')").remove();
  $iv("#sidebar div:contains('Estadísticas')").remove();
});

$iv( '#sidebar' ).each(function() {
  //$iv('#loginform').remove();
  $iv('.fieldset1').remove();
  $iv('.textwidget').remove();
  $iv('.post-social').remove();
});

$iv( '#seguidores' ).each(function() {
  $iv(this).remove();
});
...
function fuckredirect(){
  ...
  var string = $iv( this ).attr('href').replace(/.+\.to\/\w\//gi,'');
